...
% PD gain
kp = 0.5;   kd = 1; k = 50;    % two_link
%kp = kp; kd = 1; k = k;    % worst
%kp = 0.5;   kd = 1; k = 100;    % two_link_df
K = [k, 0; 0, k];
Kp = [kp, 0; 0, kp];
Kd = [kd, 0; 0, kd];
...
for i = t0 : s_time : tf
    ...
    % Solve the IK using the CLIK
    qdot_d = pinv(Ja)*(xdot_d + K*(x_d-x));
    q_d = q_d + qdot_d * s_time;
       

    % Control part to be designed : PDG %
    u = Kp*(q_d-q) - Kd*(qdot-qdot_d) + G;
